 ...
 * 
 * What this file is:
 * This is the final component located at the absolute bottom of the website.
 * 
 * What it's responsible for:
 * It provides quick navigation links back up to the top, displays a massive brutalist 
 * text logo, and shows dynamic copyright information.
 * 
 * Where it's used:
 * Rendered in `App.jsx`, placed completely outside the `<main>` tag at the bottom.
 */

export default function Footer() {
  return (
    // The footer container uses the red background and paper texture global classes
    <footer className="bg-red paper-texture text-black py-10 md:py-16 px-4 sm:px-6 lg:px-8 border-t-[3px] border-black overflow-hidden relative">
      ...
      {/* 
        ...
        This is a purely aesthetic element. It renders the word "PORTFOLIO" incredibly large
        and centers it perfectly using absolute positioning and negative translation (-50%).
        'pointer-events-none' ensures it can't be clicked, and 'select-none' prevents text highlighting.
      */}
      ...
      <div className="max-w-[1500px] mx-auto relative z-10 flex flex-col justify-center">
        ...
        {/* BRUTALIST ARROW NAVIGATION (Top row of links) */}
        <div className="flex flex-wrap justify-center gap-6 md:gap-10 mb-12 md:mb-16 font-mono text-xs md:text-sm tracking-[0.2em] font-bold uppercase text-black/80">
          {/* Each link uses an anchor tag that points to the ID of a section, jumping the user up the page */}
        ...
        {/* MAIN REFERENCE LAYOUT ROW (The massive "ABINASH" text and surrounding info) */}
        <div className="relative flex items-center justify-center w-full">

          {/* Left Text (Hidden on small screens, absolute on huge screens) */}
          <div className="hidden xl:block absolute left-0 font-mono text-[10px] md:text-xs tracking-widest uppercase text-black/80 font-bold z-20">
            {/* We use new Date().getFullYear() so the copyright year automatically updates every year! */}
            © {new Date().getFullYear()} ABINASH. ALL RIGHTS RESERVED.
          </div>

          {/* Center Text (The big name) */}
          <div className="flex items-center justify-center gap-4 md:gap-6 w-full z-10">
            {/* Left horizontal line */}
            <div className="h-[3px] bg-black w-6 sm:w-10 lg:w-12 xl:w-4 hidden sm:block"></div>
          ...
          {/* Right Text (Hidden on small screens, absolute on huge screens) */}
          <div className="hidden xl:flex absolute right-0 font-mono text-[10px] md:text-xs tracking-widest uppercase items-center gap-6 text-black/80 font-bold z-20">
            <div className="flex items-center gap-2">DESIGNED & BUILT WITH PASSION <span className="text-white text-base leading-none">♥</span></div>
        ...
        {/* 
          ...
          Because the absolute positioning above breaks on phones, we hide the absolute text 
          and render a stacked, flexible version of the same text down here just for smaller screens.
        */}
